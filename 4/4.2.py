    ...
    def is_correct_by_filter(self, filter: str) -> bool:
        if filter == "":
            ...
            if key == "Оклад":
                ...
                return Data_Dictionary.dic_salary_currency[self.field_dictionary['salary_currency']] == value
            elif key == "Навыки":
                ...
                return self.field_dictionary[inv_dic_filter[key]] == value
...
def add_to_table(table: PrettyTable, data_vacancies: list):
    count = 0
...
def is_correct_filter(filter: str) -> bool:
    if filter == "":
        return True
    if not filter.__contains__(": "):
...
table.field_names = Data_Dictionary.dic_naming.values()
table.max_width = 20
table.align = 'l'
...
if not is_correct_filter(filter_for_table):
    exit()

vacancies_array, headers_array = csv_reader(file_name, filter_for_table)

if len(vacancies_array) == 0:
    print("Ничего не найдено")
else:
    add_to_table(table, vacancies_array)
